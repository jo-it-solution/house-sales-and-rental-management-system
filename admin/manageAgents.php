...
if(isset($_POST['delete'])){
   ...
   if($verify_delete->rowCount() > 0){
      ...
      $delete_user = $conn->prepare("DELETE FROM `agents` WHERE id = ?");
...
<section class="grid">
   ...
   <?php
      if(isset($_POST['search_box']) OR isset($_POST['search_btn'])){
         ...
         $select_users = $conn->prepare("SELECT * FROM `agents` WHERE name LIKE '%{$search_box}%' OR number LIKE '%{$search_box}%' OR email LIKE '%{$search_box}%'");
         $select_users->execute();
      }else{
         $select_users = $conn->prepare("SELECT * FROM `agents`");
         $select_users->execute();
      }
